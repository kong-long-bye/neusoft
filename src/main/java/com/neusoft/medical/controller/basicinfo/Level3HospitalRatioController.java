...
@Api(tags = "三级医院报销比例维护")
@RestController
@RequestMapping("/api/level3-hospital-ratios")
public class Level3HospitalRatioController {

    @Autowired
    private HospitalReimbursementService hospitalReimbursementService;

    private static final String HOSPITAL_LEVEL = "三级";

    /**
     * 获取三级医院报销比例
     */
    @ApiOperation(value = "获取三级医院报销比例", notes = "根据人员类别获取三级医院报销比例配置")
     ...
     * 根据ID查询三级医院报销比例信息
     */
    @ApiOperation(value = "查询三级医院报销比例详情", notes = "根据报销比例ID查询详细信息")
    ...
    public Result<HospitalReimbursementVO> getById(@PathVariable Integer ratioId) {
        ...
        return ratio != null ? Result.success(ratio) : Result.error("三级医院报销比例信息不存在");
    }

    /**
     * 新增费用区间
     */
    @ApiOperation(value = "新增三级医院费用区间", notes = "为三级医院新增费用区间和对应的报销比例")
    @PostMapping
    public Result<Void> save(@ApiParam(value = "三级医院报销比例信息", required = true) @Valid @RequestBody HospitalReimbursementVO hospitalReimbursementVO) {
        try {
            // 强制设置为三级医院
     ...
     * 修改报销比例
     */
    @ApiOperation(value = "修改三级医院报销比例", notes = "根据报销比例ID修改三级医院报销比例配置")
    @ApiImplicitParam(name = "ratioId", value = "报销比例ID", required = true, dataType = "int", paramType = "path", example = "1")
    @PutMapping("/{ratioId}")
    public Result<Void> updateById(
            @PathVariable Integer ratioId,
            @ApiParam(value = "三级医院报销比例信息", required = true) @Valid @RequestBody HospitalReimbursementVO hospitalReimbursementVO) {
     ...
     * 删除费用区间
     */
    @ApiOperation(value = "删除三级医院费用区间", notes = "根据报销比例ID删除三级医院费用区间配置")
    @ApiImplicitParam(name = "ratioId", value = "报销比例ID", required = true, dataType = "int", paramType = "path", example = "1")
     ...
     * 检查费用区间是否重叠
     */
    @ApiOperation(value = "检查三级医院费用区间是否重叠", notes = "检查指定的费用区间是否与现有配置重叠")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "peopleType", value = "人员类别", required = true, dataType = "string", paramType = "query", example = "1"),
            @ApiImplicitParam(name = "minPayLevel", value = "起付金额", required = true, dataType = "string", paramType = "query", example = "1000"),
            @ApiImplicitParam(name = "maxPayLevel", value = "等级线", required = true, dataType = "string", paramType = "query", example = "5000"),
